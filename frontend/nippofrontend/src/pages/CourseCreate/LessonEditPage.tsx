...
function LessonEditPage() {
  ...
  const handleSaveTitleChanges = async () => {
    try {
      const response = await axios.post(`/lesson/edit-lesson`, {
        lessonId,
        title: lessonTitle,
      });
      if (response.status === 200) {
  ...
  const handleAddBlock = async () => {
    try {
      const response = await axios.post(`/block/create-block`, {
        lessonId,
        type: newBlockType,
  ...
  const handleDeleteBlock = async (blockId: number) => {
    try {
      const response = await axios.delete(`/block/delete?blockId=${blockId}`);
      if (response.status === 200) {
        fetchBlocks();
      } else {
        console.log("Another response status", response.status);
          ...
          {isAddingBlock ? (
            <Box>
              ...
              {(newBlockType === "Video" || newBlockType === "Image") && (
                <TextField
                  type="file"
                  onChange={(e) => setNewBlockContent(e.target.files ? e.target.files[0].name : "")}
                  sx={{ width: "100%", marginBottom: "1rem" }}
                />
              )}
